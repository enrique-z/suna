...
const DEFAULT_MODEL_ID = "sonnet-3.7"; // Define default model ID (Used for UI display if no preference)
...
export const ChatInput = forwardRef<ChatInputHandles, ChatInputProps>(({
  ...
  const checkOllamaAvailability = async () => {
    try {
      ...
      console.warn("Ollama not detected locally:", e);
    }
    return false;
  };
  ...
  const modelOptions = [
    ...
    { id: "ollama/wizard-vicuna-uncensored:30b", label: "Ollama: wizard-vicuna-uncensored:30b" },
    { id: "ollama/deepseek-coder:33b", label: "Ollama: deepseek-coder:33b" },
    { id: "ollama/deepseek-r1-16k:custom", label: "Ollama: deepseek-r1-16k:custom" },
    { id: "ollama/deepseek-r1-32:custom", label: "Ollama: deepseek-r1-32:custom" },
    { id: "ollama/mannix/deepseek-coder-v2-lite-instruct:q4_0", label: "Ollama: mannix/deepseek-coder-v2-lite-instruct:q4_0" },
  ];
  
  // Initialize state with null, will be set by useEffect from localStorage or remain null
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (savedModel && modelOptions.some(option => option.id === savedModel)) {
          ...
          // If no valid model found in storage, selectedModel remains null
          console.log(`No valid model found in localStorage. Using backend default.`);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Process model selection with proper handling
    if (selectedModel !== null) {
      ...
      // If no model is selected (null), call onSubmit with only the message
      // to trigger backend default (which was set by start-suna.py)
      onSubmit(message);
    }
                ...
                <TooltipTrigger asChild>
                  <Dialog>
                    ...
                    <DialogContent className="sm:max-w-[360px] p-0 gap-0 border border-border shadow-lg">
                      ...
                      <div className="p-4">
                        <RadioGroup
                          // Use selectedModel state, fallback to DEFAULT_MODEL_ID for initial display if null
                          defaultValue={selectedModel !== null ? selectedModel : DEFAULT_MODEL_ID}
